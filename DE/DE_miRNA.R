...
par(mgp = c(4, 1, 0))
boxplot(miR, las =3, par(mar = c(5.2, 4.7, 2, 1)+ 0.1), col = c(rep("red", 2), rep("sky blue", 2), 
                                                                 rep("green", 2), rep("pink", 2), rep("yellow", 2),
                                                              rep("violet", 2)),
        main = "Normalised Boxplot", 
        ylab = "Expression Levels", xlab = "Samples", cex.axis = 0.6)


Time <- c(rep(c(0, 1, 3, 6, 10, 14), each=2))
Lev <- as.data.frame(rep(c("D0", "D1", "D3", "D6", "D10", "D14"), each= 2))
colnames(Lev) <- "targets"
U <- unique(Lev$targets)
f <- factor(Lev$targets, levels = U)
DM <- model.matrix(~0+f)
